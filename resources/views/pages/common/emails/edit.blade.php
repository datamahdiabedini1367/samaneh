...
@section('create_form')
    ...
    {{Form::open(['route' => ['email.update', [$email->id]],'method'=>'patch','id'=>'edit_email'] )  }}
    <div class="row p-2">
        {{Form::label('value','ایمیل :',['class'=>"form-group col-sm-2"])}}
        <div class="col-sm-6">
            {{Form::text('value',$email->value,['class'=>"form-control" ,'id'=>"emailNew",'placeholder'=>"ایمیل مورد نظر را وارد کنید"]) }}
        </div>
...
@section('button-page')
    <a class="btn btn-info btn-sm float-left" href="{{route("email.show",[$type,$item])}}"> بازگشت  {{$topic}}</a>
@endsection
