...
@section('create_form')
    ...
    {{Form::open(['route' => ['phone.update', [$phone->id]],'method'=>'patch','id'=>'edit_phone'] )  }}
    <div class="row p-2">
        {{Form::label('value','شماره تماس :',['class'=>"form-group col-sm-2"])}}
        <div class="col-sm-6">
            {{Form::text('value',$phone->value,['class'=>"form-control" ,'id'=>"phoneNew",'placeholder'=>"شماره تماس مورد نظر را وارد کنید"]) }}
        </div>
...
@section('button-page')
    <a class="btn btn-info btn-sm float-left" href="{{route("phone.show",[$type,$item])}}"> بازگشت  {{$topic}}</a>
@endsection
